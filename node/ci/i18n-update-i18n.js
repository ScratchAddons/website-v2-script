...
;(async () => {
	...
	// const result = spawn.sync("tx", ["pull", "--all", "--use-git-timestamps", "--skip", "--mode", "sourceastranslation"])
	// console.log(result.stdout.toString())

	// for await (const resource of Object.keys(resourcesClient)) {
	// 	console.log(chalk`Pulling {inverse ${resource.split("r:")[1]}} using the client...`)
	// 	const result = spawn.sync("tx", ["pull", "--all", "--use-git-timestamps", "--skip", "--mode", "sourceastranslation", resource.split("p:")[1].split(':r:').join('.')])
	// 	console.log(result.stdout.toString())
